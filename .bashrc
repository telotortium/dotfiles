...
__fzf_history_bash_history_sqlite__() {
  ...
  opts="--height ${FZF_TMUX_HEIGHT:-40%} --bind=ctrl-z:ignore $FZF_DEFAULT_OPTS -n2..,.. --scheme=history --tiebreak=index \"--bind=ctrl-r:toggle-sort,ctrl-d:execute(echo {} | awk '{ sub(/^[^0-9]*/, \\\"\\\", \$1); print \$1 }' | sqlite3 ${HISTDB} 'delete from command where command_id = '\"\$(cat)\"';'; echo Deleted command {}; sleep 0.5)+exclude\" $FZF_CTRL_R_OPTS +m --read0"
